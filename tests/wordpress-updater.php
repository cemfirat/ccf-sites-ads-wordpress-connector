...
$GLOBALS['ccf_release_version'] = '1.3.2';
...
function wp_remote_get(string $url, array $options): array {
    ...
    if (($options['headers']['User-Agent'] ?? '') !== 'CCF-Sites-Ads-WordPress-Updater/1.3.1') {
    ...
    return [
        'response' => ['code' => 200],
        'body' => json_encode([
            'tag_name' => 'wordpress-v' . $version,
            'html_url' => 'https://github.com/cemfirat/ccf-sites-ads-wordpress-connector/releases/tag/wordpress-v' . $version,
            'draft' => false,
            'prerelease' => false,
            ...
            'assets' => [[
                ...
                'browser_download_url' => 'https://github.com/cemfirat/ccf-sites-ads-wordpress-connector/releases/download/wordpress-v' . $version . '/ccf-sites-ads-connector.zip',
            ]],
        ], JSON_THROW_ON_ERROR),
    ];
}
...
if (!defined('CCF_SITES_ADS_PLUGIN_VERSION') || CCF_SITES_ADS_PLUGIN_VERSION !== '1.3.1') {
    throw new RuntimeException('Plugin version constant is not v1.3.1.');
}
...
if (!is_array($native) || ($native['version'] ?? '') !== '1.3.2') {
...
if (($native['package'] ?? '') !== 'https://github.com/cemfirat/ccf-sites-ads-wordpress-connector/releases/download/wordpress-v1.3.2/ccf-sites-ads-connector.zip') {
    throw new RuntimeException('Native Update URI hook accepted an unexpected package URL.');
}

$transient = (object) [
    'checked' => ['ccf-google-ads-site-connector/ccf-google-ads-site-connector.php' => '1.3.1'],
    'response' => [],
    'no_update' => [],
];
...
if (!is_object($offer) || $offer->new_version !== '1.3.2') {
...
if (!is_array($nativeCurrent) || ($nativeCurrent['version'] ?? '') !== '1.3.1') {
...
if (!is_object($noUpdate) || ($noUpdate->new_version ?? '') !== '1.3.1') {
...
if (isset($current->response['ccf-google-ads-site-connector/ccf-google-ads-site-connector.php'])) {
    throw new RuntimeException('Current version must not be offered as an update.');
}
