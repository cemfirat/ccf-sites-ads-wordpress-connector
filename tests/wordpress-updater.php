...
$GLOBALS['ccf_release_version'] = '1.3.8';
...
function wp_remote_get(string $url, array $options): array {
    ...
    if (($options['headers']['User-Agent'] ?? '') !== 'CCF-Sites-Ads-WordPress-Updater/1.3.8') throw new RuntimeException('Updater user agent is missing or stale.');
...
if (!defined('CCF_SITES_ADS_PLUGIN_VERSION') || CCF_SITES_ADS_PLUGIN_VERSION !== '1.3.8') throw new RuntimeException('Plugin version constant is not v1.3.8.');
...
if (!is_array($native) || ($native['version'] ?? '') !== '1.3.8') throw new RuntimeException('Native Update URI hook did not return the newer stable release.');
if (array_key_exists('autoupdate', $native)) throw new RuntimeException('Updater must not force an autoupdate preference.');
if (($native['package'] ?? '') !== 'https://github.com/cemfirat/ccf-sites-ads-wordpress-connector/releases/download/wordpress-v1.3.8/ccf-sites-ads-connector.zip') throw new RuntimeException('Native Update URI hook accepted an unexpected package URL.');
...
if (!is_array($nativeCurrent) || ($nativeCurrent['version'] ?? '') !== '1.3.8') throw new RuntimeException('Native Update URI hook must return current release metadata when already up to date.');
$currentTransient = (object) ['checked' => ['ccf-google-ads-site-connector/ccf-google-ads-site-connector.php' => '1.3.8'], 'response' => [], 'no_update' => []];
...
if (!is_object($noUpdate) || ($noUpdate->new_version ?? '') !== '1.3.8') throw new RuntimeException('Current plugin metadata was not preserved in no_update.');
