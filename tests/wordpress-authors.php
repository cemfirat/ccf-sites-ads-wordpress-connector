...
function user_can($user, string $capability): bool {
    return $capability === 'edit_posts' && in_array((int) $user->ID, [1, 4], true);
}
function get_author_posts_url(int $id, $nicename = null): string {
    return 'https://customer.example/author/' . rawurlencode((string) $nicename) . '/';
}
...
if (count($filters) !== 1 || $filters[0][2] !== 3) {
    throw new RuntimeException('Author response augmenter was not registered correctly.');
}

$authors = CCF_Sites_Authors::eligible_authors();
if (count($authors) !== 2 || ($authors[0]['display_name'] ?? '') !== 'Cem Cemil Firat' || ($authors[1]['display_name'] ?? '') !== 'developez') {
    throw new RuntimeException('Eligible author discovery or sorting failed.');
}
foreach ($authors as $author) {
    $keys = array_keys($author);
    sort($keys);
    $expected = ['author_url', 'can_edit_posts', 'display_name', 'id', 'slug'];
    sort($expected);
    if ($keys !== $expected) {
        throw new RuntimeException('Author discovery exposed unexpected fields.');
    }
    if (($author['can_edit_posts'] ?? false) !== true || strpos((string) ($author['author_url'] ?? ''), 'https://customer.example/author/') !== 0) {
        throw new RuntimeException('Author public identity payload is invalid.');
    }
}
...
$inventoryData = $inventory->get_data();
if (($inventoryData['data']['authors'] ?? null) !== $authors) {
    throw new RuntimeException('Inventory was not augmented with authors.');
}
...
if (!in_array('wordpress.content.author.read', $statusData['data']['capabilities'] ?? [], true)) {
    throw new RuntimeException('Author read capability was not advertised.');
}
...
if ($other->get_data() !== ['data' => ['unchanged' => true]]) {
    throw new RuntimeException('Unrelated REST responses were modified.');
}
